...
Build a **headless, AI-native mortgage platform** at `unified-cmtg.vercel.app` (Production) that unifies:
...
- **Backend**: Django on Vercel (Serverless) / External DB
- **Agents**: Run on `dell-brain` (Background Workers)
...
4. **Deploy**: GitHub -> Vercel (Auto-deploy)

### Why This Matters
- Dell-brain has GPU for ML workloads and runs agents 24/7
- Vercel ensures high-availability production hosting (no reliance on local dell-brain network)
- Centralized code prevents sync conflicts

### Machine Roles
| Machine | Role | Code Access |
|---------|------|-------------|
| **Dell-Brain** | Development & Background Agents | Read/Write |
| **Vercel** | Production Host | Deploy Only |
